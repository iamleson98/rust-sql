...
The honest ledger. Everything here is verifiable absence — `module_list`/`function_list`/`compile_options` report what is actually compiled in, the C ABI exports exactly its 124 symbols, and the fuzzers are seeded and reproducible. (No open correctness items right now: every pinned divergence class is closed and re-verified per push — the regression pins live in `tests/stateful_fuzz.rs` (`stateful_fuzz_fresh_seed_pins`), `tests/concurrent_reader_visibility.rs`, and the differential suites; fix histories are in `worklog.md`.)
