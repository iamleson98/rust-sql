...
> **Status** (all numbers from the latest CI-green run [36259628058](https://github.com/iamleson98/rust-sql/actions/runs/36259628058) @ `9d31855`, 2026-09-26, 31/31 jobs):
> **1393 tests** in the default matrix, plus sqlx / no-default / oom-injection / compat-ABI matrices on 3 OSes — and a dedicated **limit-stress** job (8 push-to-the-limit sections at 1M-row file scale) on every OS; the 2026-09-26 rounds closed the engine-generation-boundary durability class (a fresh open could straddle a live writer's WAL fold+reset and miss committed frames; a superseded generation's late teardown could fold its stale map over its successor's newer file) and landed **the SQLite-format container's incremental page-diff architecture** — per-root change epochs pick the dirty objects, each commit splices only those b-trees onto stable page identities (own previous pages → container freelist → tail), shrinkage routes into a real SQLite freelist, one per-file page-space coordinator shares the layout across sessions (per-object last-writer merge, one WAL chain), and a fresh process ADOPTS the file's layout so the first commit after a restart splices too — per-commit CPU and I/O are now **O(changed object)** where the whole image used to be re-derived every boundary; the epoch machinery is gated to SQLite-format databases, so native and in-memory hot paths pay nothing (the bench-gate row that caught the ungated cost is green again). **54 benchmark rows vs real SQLite: 53 wins, 1 parity tie, 0 losses.** Byte-identical file size, lower-or-equal peak RSS on 14/18 torture sections (the rest +1–2 MB by allocator choice). Three concurrency tiers beyond SQLite's envelope, plus `BEGIN CONCURRENT` multi-writer transactions. sqlx 0.9 native driver **and** drop-in `libsqlite3` C ABI verified with sea-orm 2.0 end to end.
...
- **SQLite-format per-commit CPU — CLOSED (2026-09-26)**: the incremental page-diff architecture landed (per-root change epochs → object splices onto stable page identities → real SQLite freelist for shrinkage → shared per-file page-space coordinator for multi-session; layout adoption from the file on reopen so the first commit after a restart splices too). Remaining follow-up: the engine's own big-blob `INSERT OR REPLACE` split corner (`leaf page cannot split: no feasible byte-aware point`, reproducible on the NATIVE container — `examples/probe_native_fuzz.rs`).
