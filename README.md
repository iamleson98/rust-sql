...
- **[FIXED 2026-09-20] `BEGIN CONCURRENT` live-engine read visibility**: the 2026-09-19 pinned divergence (every live-engine reader could observe a consistent PARTIAL table — COUNT(*) = 467 of 10000 — during a multi-writer burst) was a three-step bookkeeping failure, all engine-side root maps: (1) a FAILED concurrent commit deregisters the transaction before the error surfaces, so the caller's cleanup `ROLLBACK` fell through to the plain-rollback epilogue, which restored a `None` maps snapshot as `empty_maps()` — wiping every live root override; (2) `table_root()` then fell back to the catalog's DDL-time root — an interior node after the seed's splits — and served its subtree as the whole table; (3) `publish_concurrent_maps` swapped the committing transaction's BEGIN-time overlay in wholesale, regressing roots sibling transactions had committed. Fixes: the no-snapshot rollback keeps the current maps; the publish merges only the transaction's DELTA (against a recorded begin-time base) with every root value reconciled through the pager's committed-root view plus the merge fixups; the concurrent-owner schema-row sync rewrites only the transaction's own root moves. All four-writer shapes now run clean with full commit progress — `tests/concurrent_reader_visibility.rs` (un-ignored, writer-progress asserts), root-resolution tracing under `RSQL_DBG_CW=1` (`Database::dbg_root_state`).
- **[FIXED 2026-09-20] WAL mode now persists (SQLite parity)**: `journal_mode=WAL` is recorded in the native file header (offset 72) — a reopen of a WAL-mode file comes up in WAL mode (SQLite's persistent-WAL header semantics; previously every fresh handle silently reverted to DELETE mode and `BEGIN CONCURRENT` failed with "requires journal_mode=WAL"). A first ever write to a WAL-mode file from a SECOND independent handle is now guarded by a process-wide WAL writer lease: it fails fast with SQLITE_BUSY (the same behavior a leaked sqlite3* connection exhibits through its POSIX locks) instead of interleaving sidecar appends — concurrent multi-handle WRITES to a native file remain out of contract by design; use one engine (the sqlx pool's per-path shared engine) for multi-connection workloads. Pinned: `wal_mode_persists_across_reopen` + `concurrent_second_writer_handle_gets_busy_not_corruption`.
