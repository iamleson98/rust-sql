...
fn seeded(db: &mut Database, n: i64) {
    db.execute(
        "CREATE TABLE t (id INTEGER PRIMARY KEY, b BLOB, s TEXT, v INTEGER)",
        [],
    )
    .unwrap();
...
fn overflow_mixed_rows_scan_order() {
    ...
    let rows = db
        .query("SELECT v, length(b), length(s) FROM t ORDER BY id", [])
        .unwrap();
...
fn overflow_range_scan() {
    ...
    let rows = db
        .query(
            "SELECT length(b) FROM t WHERE id BETWEEN 10 AND 19 ORDER BY id",
            [],
        )
        .unwrap();
...
fn overflow_update_paths() {
    ...
    db.execute(
        "UPDATE t SET b = ? WHERE id = 5",
        [Value::Blob(vec![9u8; 100])],
    )
    .unwrap();
    let rows = db
        .query("SELECT length(b) FROM t WHERE id = 5", [])
        .unwrap();
    assert_eq!(rows[0][0], Value::Integer(100));
    // Same-size in-place patch of an in-page row among spilled rows.
    db.execute(
        "UPDATE t SET b = ? WHERE id = 6",
        [Value::Blob(vec![3u8; 100])],
    )
    .unwrap();
    db.execute(
        "UPDATE t SET b = ? WHERE id = 6",
        [Value::Blob(vec![4u8; 100])],
    )
    .unwrap();
    ...
    let rows = db
        .query("SELECT COUNT(*), SUM(length(b)) FROM t", [])
        .unwrap();
...
fn overflow_delete_reclaims_and_stays_correct() {
    ...
    let rows = db
        .query("SELECT length(b) FROM t WHERE id = 5", [])
        .unwrap();
    assert_eq!(rows[0][0], Value::Integer(4 + 1));
    // Deleting a spilled row twice is a no-op.
    db.execute("DELETE FROM t WHERE id = 3", [Value::Integer(3)])
        .unwrap();
    // Freelist pages are reused by new spilled rows (no unbounded growth).
    db.execute("PRAGMA page_count", []).unwrap();
    for i in 0..10i64 {
        db.execute(
            "INSERT INTO t (b, s, v) VALUES (?, ?, ?)",
            [
                Value::Blob(vec![1u8; 2 * 4096]),
                Value::Text("z".repeat(64).into()),
...
fn overflow_rollback() {
    ...
    db.execute("DELETE FROM t WHERE id = 2", [Value::Integer(2)])
        .unwrap();
    db.execute("ROLLBACK", []).unwrap();
    let rows = db.query("SELECT COUNT(*) FROM t", []).unwrap();
    assert_eq!(rows[0][0], Value::Integer(10));
    let rows = db
        .query("SELECT length(b) FROM t WHERE id = 2", [])
        .unwrap();
    assert_eq!(rows[0][0], Value::Integer(1 + 1));
    // Post-rollback writes work (chain bookkeeping is clean).
    db.execute(
        "INSERT INTO t (b, s, v) VALUES (?, ?, ?)",
        [
            Value::Blob(vec![5u8; 4097]),
            Value::Text("p".into()),
            Value::Integer(600),
        ],
    )
    .unwrap();
    let rows = db
        .query("SELECT length(b) FROM t WHERE v = 600", [])
        .unwrap();
...
fn overflow_persistence_across_reopen() {
    ...
    let rows = db
        .query("SELECT length(b), length(s) FROM t WHERE id = 12", [])
        .unwrap();
    assert_eq!(rows[0][0], Value::Integer(11 + 1));
...
fn overflow_with_index() {
    let mut db = Database::open_in_memory().unwrap();
    db.execute(
        "CREATE TABLE t (id INTEGER PRIMARY KEY, v INTEGER, b BLOB)",
        [],
    )
    .unwrap();
    ...
    for i in 0..20i64 {
        db.execute(
            ...
            [
                Value::Integer(i % 5),
                Value::Blob(vec![(i % 7) as u8; (i as usize % 9000) + 1]),
            ],
        )
        .unwrap();
    }
    ...
    let rows = db
        .query("SELECT length(b) FROM t WHERE v = 2 ORDER BY id", [])
        .unwrap();
    for (k, r) in rows.iter().enumerate() {
        let i = 2 + 5 * k;
        assert_eq!(r[0], Value::Integer(((i % 9000) + 1) as i64));
...
fn overflow_replace_and_upsert() {
    ...
    .unwrap();
    let rows = db.query("SELECT length(b) FROM t", []).unwrap();
    assert_eq!(rows[0][0], Value::Integer(2 * 4096_i64));
    ...
    .unwrap();
    let rows = db.query("SELECT length(b) FROM t", []).unwrap();
    assert_eq!(rows[0][0], Value::Integer(4 * 4096_i64));
}
...
fn overflow_text_functions() {
    ...
    let rows = db
        .query(
            "SELECT length(s), upper(substr(s, 1, 6)), substr(s, -3) FROM t",
            [],
        )
        .unwrap();
