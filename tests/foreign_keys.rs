...
fn fk_db() -> Database {
    ...
    .unwrap();
    db.execute("INSERT INTO parent (name) VALUES ('a'), ('b')", [])
        .unwrap();
    db
}
...
fn fk_on_rejects_orphan_insert() {
    ...
    db.execute("INSERT INTO child (pid) VALUES (NULL)", [])
        .unwrap();
...
fn fk_on_delete_cascade() {
    ...
    .unwrap();
    db.execute("PRAGMA foreign_keys = ON", []).unwrap();
    db.execute("INSERT INTO p (id) VALUES (1), (2)", [])
        .unwrap();
    db.execute("INSERT INTO c (pid) VALUES (1), (1), (2)", [])
        .unwrap();
...
fn fk_on_delete_set_null() {
    ...
    db.execute("INSERT INTO p (id) VALUES (1), (2)", [])
        .unwrap();
    db.execute("INSERT INTO c (pid) VALUES (1), (2)", [])
        .unwrap();
    db.execute("DELETE FROM p WHERE id = 1", []).unwrap();
...
fn fk_on_delete_set_default() {
    ...
    db.execute("INSERT INTO p (id) VALUES (0), (1)", [])
        .unwrap();
    ...
    assert_eq!(
        ...
        "child key must become the default"
    );
}
...
fn fk_composite_key_enforcement() {
    ...
    // Exact match passes.
    db.execute("INSERT INTO ct (x, y) VALUES (1, 2)", [])
        .unwrap();
...
fn fk_implicit_parent_pk_reference() {
    ...
    .unwrap();
    db.execute("PRAGMA foreign_keys = ON", []).unwrap();
    db.execute("INSERT INTO parent (id, name) VALUES (5, 'e')", [])
        .unwrap();
    db.execute("INSERT INTO child (pid) VALUES (5)", [])
        .unwrap();
...
fn fk_cascade_is_recursive() {
    ...
    // through both levels.
    let mut db = Database::open_in_memory().unwrap();
    db.execute("CREATE TABLE gp (id INTEGER PRIMARY KEY)", [])
        .unwrap();
    db.execute(
    ...
    db.execute("INSERT INTO p (id, gpid) VALUES (10, 1)", [])
        .unwrap();
    db.execute("INSERT INTO c (id, pid) VALUES (100, 10)", [])
        .unwrap();
    ...
    assert_eq!(
        db.query("SELECT COUNT(*) FROM c", []).unwrap()[0][0],
        Value::Integer(0)
    );
}
...
fn fk_text_parent_key() {
    ...
    .unwrap();
    db.execute("PRAGMA foreign_keys = ON", []).unwrap();
    db.execute("INSERT INTO users (email, name) VALUES ('a@x', 'Ann')", [])
        .unwrap();
    db.execute("INSERT INTO orders (email) VALUES ('a@x')", [])
        .unwrap();
...
fn fk_delete_no_pk_table_with_fks() {
    ...
    assert_eq!(
        db.query("SELECT COUNT(*) FROM parent", []).unwrap()[0][0],
        Value::Integer(0)
    );
}
...
fn fk_index_maintenance_on_cascade_delete() {
    ...
    db.execute("INSERT INTO p (id) VALUES (1), (2)", [])
        .unwrap();
    db.execute(
        "INSERT INTO c (pid, tag) VALUES (1, 'one'), (1, 'uno'), (2, 'two')",
        [],
    )
    .unwrap();
    db.execute("DELETE FROM p WHERE id = 1", []).unwrap();
