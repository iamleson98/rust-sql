...
fn in_list_semantics_and_planning() {
    ...
    db.execute("CREATE TABLE t (id INTEGER PRIMARY KEY, v TEXT, k INTEGER)", []).unwrap();
    ...
    let rows = db.query("SELECT v FROM t WHERE id IN (5, 3, 9, 100, 3)", []).unwrap();
    let got: Vec<String> = rows.iter().map(|r| r[0].as_text()).collect();
    assert_eq!(got, vec!["v3", "v5", "v9", "v100"], "rowid IN: {:?}", got);

    // Rowid IN with params.
    let rows = db.query("SELECT id FROM t WHERE id IN (?, ?, ?) ORDER BY id", [Value::Integer(7), Value::Integer(2), Value::Integer(7)]).unwrap();
    assert_eq!(rows.len(), 2);

    // Rowid IN with non-integer + NULL members.
    let rows = db.query("SELECT id FROM t WHERE id IN (1, 'x', NULL, 2.5, 3)", []).unwrap();
    assert_eq!(rows.len(), 2, "non-int members skipped: {:?}", rows);

    // Rowid IN with residual.
    let rows = db.query("SELECT id FROM t WHERE id IN (1,2,3,4,5,6) AND v = 'v4'", []).unwrap();
    assert_eq!(rows.len(), 1);
    assert_eq!(rows[0][0], Value::Integer(4));

    // NOT IN keeps scan semantics (all minus listed).
    let rows = db.query("SELECT COUNT(*) FROM t WHERE id NOT IN (1,2,3)", []).unwrap();
    ...
    let rows = db.query("SELECT COUNT(*) FROM t WHERE k IN (1, 3, 5)", []).unwrap();
    ...
    let rows = db.query("SELECT COUNT(*) FROM t WHERE k IN (2, 4) AND id > 500", []).unwrap();
    assert_eq!(rows[0][0], Value::Integer(100), "residual count");

    // Index IN with params.
    let rows = db.query("SELECT COUNT(*) FROM t WHERE k IN (?, ?)", [Value::Integer(1), Value::Integer(2)]).unwrap();
    assert_eq!(rows[0][0], Value::Integer(200));

    // UPDATE with rowid IN.
    db.execute("UPDATE t SET v = 'upd' WHERE id IN (10, 20, 30)", []).unwrap();
    ...
    // EXPLAIN shape works.
    let _ = db.query("EXPLAIN QUERY PLAN SELECT id FROM t WHERE id IN (1, 2)", []).unwrap();
    let _ = db.query("EXPLAIN QUERY PLAN SELECT id FROM t WHERE k IN (1, 2)", []).unwrap();
