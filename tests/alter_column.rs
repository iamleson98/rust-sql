...
fn rename_column_basic() {
    ...
    .unwrap();
    db.execute("INSERT INTO t (name, val) VALUES ('a', 1), ('b', 2)", [])
        .unwrap();
...
fn rename_column_survives_reopen() {
    ...
    let _ = std::fs::remove_file(&path);
    {
        ...
        .unwrap();
        db.execute("INSERT INTO t (x, y) VALUES (10, 'ten')", [])
            .unwrap();
        db.execute("ALTER TABLE t RENAME COLUMN x TO alpha", [])
            .unwrap();
        db.flush().unwrap();
    }
...
fn rename_column_updates_indexes_and_check() {
    ...
    .unwrap();
    db.execute(
        "INSERT INTO t (v, w) VALUES (1, 'a'), (5, 'b'), (9, 'c')",
        [],
    )
    .unwrap();
    db.execute("CREATE INDEX idx_v ON t(v)", []).unwrap();

    db.execute("ALTER TABLE t RENAME COLUMN v TO score", [])
        .unwrap();
    ...
    assert!(db
        .execute("INSERT INTO t (score, w) VALUES (-1, 'bad')", [])
        .is_err());
    db.execute("INSERT INTO t (score, w) VALUES (7, 'ok')", [])
        .unwrap();

    // The index survived with correct contents.
    let rows = db
        .query(
            "SELECT id FROM t WHERE score BETWEEN 4 AND 8 ORDER BY score",
            [],
        )
        .unwrap();
...
fn rename_column_rewrites_fk_references() {
    ...
    .unwrap();
    db.execute("INSERT INTO p (code) VALUES ('x1')", [])
        .unwrap();
    db.execute("INSERT INTO c (pcode) VALUES ('x1')", [])
        .unwrap();

    db.execute("ALTER TABLE p RENAME COLUMN code TO tag", [])
        .unwrap();

    // FK still enforced with the new parent column name.
    db.execute("PRAGMA foreign_keys = ON", []).unwrap();
    assert!(db
        .execute("INSERT INTO c (pcode) VALUES ('nope')", [])
        .is_err());
    db.execute("INSERT INTO c (pcode) VALUES ('x1')", [])
        .unwrap();
...
fn rename_column_trigger_body() {
    let mut db = memdb();
    db.execute(
        "CREATE TABLE t (id INTEGER PRIMARY KEY, v INTEGER, log TEXT)",
        [],
    )
    .unwrap();
    db.execute(
    ...
    .unwrap();
    db.execute("INSERT INTO t (v, log) VALUES (10, 'orig')", [])
        .unwrap();

    // Renaming v must rewrite NEW.v and the INSERT column list.
    db.execute("ALTER TABLE t RENAME COLUMN v TO amount", [])
        .unwrap();
    db.execute("INSERT INTO t (amount, log) VALUES (6, 'second')", [])
        .unwrap();
...
fn rename_column_view_sql() {
    ...
    db.execute("CREATE VIEW big AS SELECT id FROM t WHERE v > 1", [])
        .unwrap();

    db.execute("ALTER TABLE t RENAME COLUMN v TO val", [])
        .unwrap();
...
fn rename_column_errors() {
    ...
    // Unknown column.
    assert!(db
        .execute("ALTER TABLE t RENAME COLUMN zz TO q", [])
        .is_err());
...
fn drop_column_basic() {
    ...
    .unwrap();
    db.execute(
        "INSERT INTO t (name, val, score) VALUES ('a', 1, 1.5), ('b', 2, 2.5)",
        [],
    )
    .unwrap();
...
fn drop_column_middle_position() {
    ...
    .unwrap();
    db.execute(
        "INSERT INTO t (a, b, c) VALUES (1, 'one', 1.25), (2, 'two', 2.5)",
        [],
    )
    .unwrap();
...
fn drop_column_alias_after_dropped() {
    ...
    let mut db = memdb();
    db.execute(
        "CREATE TABLE t (name TEXT, id INTEGER PRIMARY KEY, v INTEGER)",
        [],
    )
    .unwrap();
    db.execute("INSERT INTO t (name, v) VALUES ('a', 10)", [])
        .unwrap();
...
fn drop_column_errors() {
    ...
    db.execute("INSERT INTO t (a, b, ck) VALUES (1, 2, 3)", [])
        .unwrap();
    // Unknown.
...
fn drop_column_referenced_by_fk() {
    let mut db = memdb();
    db.execute("CREATE TABLE p (id INTEGER PRIMARY KEY, code TEXT)", [])
        .unwrap();
    db.execute(
        "CREATE TABLE c (id INTEGER PRIMARY KEY, pcode TEXT REFERENCES p(code))",
        [],
...
fn drop_column_referenced_by_view() {
    ...
    db.execute("CREATE VIEW vv AS SELECT id FROM t WHERE v > 0", [])
        .unwrap();
