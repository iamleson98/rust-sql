...
fn main() {
    ...
    println!(
        "BETWEEN 0..100 LIMIT 1 : {:>7.2} µs/q (expect ~few µs if pushdown fires)",
    ...
    for _ in 0..n {
        let _ = db.query(
            "SELECT a FROM bench WHERE a BETWEEN 4900 AND 5000 LIMIT 1",
            (),
        );
    }
    println!(
        "BETWEEN 4900.. LIMIT 1  : {:>7.2} µs/q (expect ~full-scan cost)",
        t.elapsed().as_secs_f64() * 1e6 / n as f64
    );
    ...
    println!(
        "BETWEEN 0..100 no LIMIT : {:>7.2} µs/q",
        t.elapsed().as_secs_f64() * 1e6 / n as f64
    );
}
