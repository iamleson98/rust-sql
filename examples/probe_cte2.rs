...
fn main() {
    ...
    let r = db
        .query(
            "WITH a AS (SELECT val FROM t WHERE val > 30) SELECT SUM(val) FROM a",
            [],
        )
        .unwrap();
    ...
    let r = db
        .query(
            "SELECT (SELECT MAX(val) FROM t) - (SELECT MIN(val) FROM t)",
            [],
        )
        .unwrap();
    println!(
        "scalar arith no cte: {:?} (expect 90) err={:?}",
        r,
        db.query(
            "SELECT (SELECT MAX(val) FROM t) - (SELECT MIN(val) FROM t)",
            []
    ...
    match db.query(
        "WITH inner1 AS (SELECT val FROM t WHERE val > 80) SELECT COUNT(*) FROM inner1",
        [],
    ) {
    ...
    // Same without CTE:
    match db.query(
        "SELECT COUNT(*) FROM t WHERE t.val IN (SELECT val FROM t WHERE val > 50)",
        [],
    ) {
