...
fn main() {
    ...
    db.execute("CREATE TABLE t (id INTEGER PRIMARY KEY, val INTEGER, score REAL, name TEXT)", []).unwrap();
    db.execute("BEGIN", []).unwrap();
    for i in 1..=10_000i64 {
        let sql = format!("INSERT INTO t (val, score, name) VALUES ({}, {}, 'user{}')", i, i as f64, i);
    ...
    let ps = db.page_size();
    println!("rustqlite: file={} pages={} page_size={} bytes/page={:.0} fill={:.1}%", sz, pc, ps, sz as f64 / pc as f64, (sz - 100) as f64 / (pc as f64 * ps as f64) * 100.0);
    ...
    let pc2: i64 = conn.query_row("PRAGMA page_count", [], |r| r.get(0)).unwrap();
    println!("sqlite16k: file={} pages={} bytes/page={:.0}", sz2, pc2, sz2 as f64 / pc2 as f64);
