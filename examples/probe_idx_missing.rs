...
fn main() {
    for n in [1000usize, 2000] {
        ...
        let total_idx: i64 = db.query(
            "SELECT COUNT(*) FROM t WHERE cat >= 'a'", []).unwrap()[0][0].as_integer();
        let rows_t: i64 = db.query("SELECT COUNT(*) FROM t", []).unwrap()[0][0].as_integer();
        println!("n={}: table rows={}  index-visible(cat>='a')={}  {}",
            n, rows_t, total_idx,
            if total_idx == rows_t { "OK".to_string() } else { format!("MISSING {}", rows_t - total_idx) });

        // Which rowids are missing? Compare per-cat counts.
        for cat in ["a", "b", "c"] {
            let cnt = db.query("SELECT COUNT(*) FROM t WHERE cat = ?",
                [Value::Text(cat.into())]).unwrap()[0][0].as_integer();
            println!("   cat={} -> {}", cat, cnt);
        }
        ...
        dbf.execute("CREATE TABLE t (id INTEGER PRIMARY KEY, cat TEXT, val INTEGER)", []).unwrap();
        dbf.execute("BEGIN", []).unwrap();
        for i in 1..=n as i64 {
            let cat = if i % 3 == 0 { "a" } else if i % 3 == 1 { "b" } else { "c" };
            dbf.execute("INSERT INTO t (cat, val) VALUES (?, ?)",
                [Value::Text(cat.into()), Value::Integer(i)]).unwrap();
        }
        dbf.execute("COMMIT", []).unwrap();
        dbf.execute("CREATE INDEX idx_cat ON t(cat)", []).unwrap();
        let total_idxf: i64 = dbf.query(
            "SELECT COUNT(*) FROM t WHERE cat >= 'a'", []).unwrap()[0][0].as_integer();
        let rows_tf: i64 = dbf.query("SELECT COUNT(*) FROM t", []).unwrap()[0][0].as_integer();
        println!("   file-backed: table={} index-visible={} {}",
            rows_tf, total_idxf,
            if total_idxf == rows_tf { "OK".to_string() } else { format!("MISSING {}", rows_tf - total_idxf) });
        println!();
    }
}
