...
fn main() {
    ...
    let d = start.elapsed();
    let stats = (0,0);
    println!("random indexed: {:?} ({} ns/row) append hits={} misses={}",
        d, d.as_nanos() as f64 / 10_000.0, stats.0, stats.1);
    ...
    let sorted: Vec<i64> = { let mut v = vals.clone(); v.sort(); v };
    let start = Instant::now();
    ...
    println!("sorted indexed: {:?} ({} ns/row)", d2, d2.as_nanos() as f64 / 10_000.0);
}
