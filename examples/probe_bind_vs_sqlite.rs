...
/// One (label, bind-constructor, expected-values) query case.
type QueryCase = (
    &'static str,
    Box<dyn Fn(i64) -> Vec<Value>>,
    Box<dyn Fn(i64) -> Vec<i64>>,
);
...
fn main() {
    ...
    conn.execute("BEGIN", []).unwrap();
    {
        let mut stmt = conn
            ...
            .unwrap();
        for i in 0..5000i64 {
            stmt.execute(rusqlite::params![i, i as f64 * 0.5, format!("name-{i}")])
                .unwrap();
        }
    }
    ...
    let n = 2000i64;
    let queries: Vec<QueryCase> = vec![
        (
            "bind BETWEEN + COUNT/AVG",
            Box::new(|i: i64| {
                vec![
                    Value::Integer((i * 2) % 4000),
                    Value::Integer((i * 2) % 4000 + 50),
                ]
            }),
