...
fn main() {
    ...
    for i in 1..=n {
        let sql = format!(
            "INSERT INTO t (name, val) VALUES ('item{}', {})",
            i, i * 2
        );
    ...
    println!("Inserted {} rows in {:.2?} ({:.0} rows/sec)", n, elapsed, n as f64 / elapsed.as_secs_f64());
    ...
    let elapsed = start.elapsed();
    if rows.is_empty() {
        println!("Point lookup in {:.2?}: (no rows — rowid index not yet used)", elapsed);
    } else {
