...
fn main() {
    ...
    for _ in 0..50 { let _ = db.query(sql, [Value::Integer(500)]).unwrap(); }
    let start = Instant::now();
    let _ = db.query(sql, [Value::Integer(500)]).unwrap();
    println!("warm query (no storm):        {:>7.1} us", us(start.elapsed()));
    ...
    let start = Instant::now();
    let _ = db.query(sql, [Value::Integer(500)]).unwrap();
    println!("after 100-row COMMIT:         {:>7.1} us", us(start.elapsed()));
    ...
    let start = Instant::now();
    let _ = db.query(sql, [Value::Integer(500)]).unwrap();
    println!("after 50k-row COMMIT:         {:>7.1} us", us(start.elapsed()));
    // second query right after
    let start = Instant::now();
    let _ = db.query(sql, [Value::Integer(501)]).unwrap();
    println!("  2nd query after storm:      {:>7.1} us", us(start.elapsed()));
    let start = Instant::now();
    let _ = db.query(sql, [Value::Integer(502)]).unwrap();
    println!("  3rd query after storm:      {:>7.1} us", us(start.elapsed()));
    ...
    let start = Instant::now();
    let _ = db.query(sql, [Value::Integer(500)]).unwrap();
    println!("after 50k COMMIT + alloc churn:{:>6.1} us", us(start.elapsed()));
    ...
    println!("fresh stmt after storm:       {:>7.1} us", us(start.elapsed()));

    // --- page-touch cost: full scan right after a storm (all pages) ---
    let start = Instant::now();
    let _ = db.query("SELECT COUNT(*) FROM t", []).unwrap();
    println!("COUNT(*) after storm:         {:>7.1} us", us(start.elapsed()));
    ...
    let start = Instant::now();
    db.execute("CREATE INDEX idx_val ON t(val)", []).unwrap();
    println!("CREATE INDEX (110k rows):     {:>7.1} us", us(start.elapsed()));
    let start = Instant::now();
    let _ = db.query(sql, [Value::Integer(500)]).unwrap();
    println!("  1st query after CREATE INDEX:{:>6.1} us", us(start.elapsed()));
    let start = Instant::now();
    let _ = db.query(sql, [Value::Integer(500)]).unwrap();
    println!("  2nd query after CREATE INDEX:{:>6.1} us", us(start.elapsed()));
}
