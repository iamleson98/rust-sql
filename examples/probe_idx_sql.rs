fn main() {
    ...
    for v in (2..=2000i64).step_by(2) {
        let rows = db.query("SELECT id FROM t WHERE val = ?", [rustqlite::Value::Integer(v)]).unwrap();
        if rows.len() != 1 {
            missing.push(v);
        }
    }
    ...
    if let Some(v) = missing.first() {
        let a = db.query("SELECT COUNT(*) FROM t WHERE val = ?", [rustqlite::Value::Integer(*v)]).unwrap();
        println!("val={} count via index: {:?}", v, a[0][0]);
        let b = db.query("SELECT COUNT(*) FROM t WHERE val + 0 = ?", [rustqlite::Value::Integer(*v)]).unwrap();
        println!("val={} count via full scan: {:?}", v, b[0][0]);
        let c = db.query("SELECT id FROM t WHERE val BETWEEN ? AND ?", [rustqlite::Value::Integer(*v), rustqlite::Value::Integer(*v)]).unwrap();
        println!("val={} BETWEEN count: {}", v, c.len());
        let d = db.query("SELECT id FROM t WHERE val > ? AND val < ?", [rustqlite::Value::Integer(*v - 1), rustqlite::Value::Integer(*v + 1)]).unwrap();
