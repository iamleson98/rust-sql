...
fn main() {
    ...
    db.execute("CREATE TABLE t (id INTEGER PRIMARY KEY, name TEXT, val REAL)", []).unwrap();
    db.execute("INSERT INTO t (val, name, id) VALUES (-2.5, 'it''s «héllo»', 7)", []).unwrap();
    ...
    db.execute("INSERT INTO t (name) VALUES ('partial')", []).unwrap();
    let rows = db.query("SELECT id, val FROM t WHERE name = 'partial'", []).unwrap();
    ...
    db.execute("CREATE TABLE nn (a INTEGER NOT NULL, b TEXT)", []).unwrap();
    let r = db.execute("INSERT INTO nn (b) VALUES ('x')", []);
    assert!(r.is_err(), "NOT NULL violation must error");
    println!("[ok] NOT NULL enforcement: {:?}", r.err().map(|e| e.to_string()).unwrap_or_default());

    // 4. UNIQUE index maintenance through fast path.
    db.execute("CREATE TABLE u (id INTEGER PRIMARY KEY, email TEXT UNIQUE)", []).unwrap();
    db.execute("INSERT INTO u (email) VALUES ('a@x.com')", []).unwrap();
    let r = db.execute("INSERT INTO u (email) VALUES ('a@x.com')", []);
    assert!(r.is_err(), "UNIQUE violation must error");
    db.execute("INSERT INTO u (email) VALUES ('b@x.com')", []).unwrap();
    ...
    db.execute("INSERT INTO t (id, val) VALUES (0x10, 1e3)", []).unwrap();
    ...
    db.execute("insert into T (NAME) values ('cased')", []).unwrap();
    let n = db.query("SELECT COUNT(*) FROM t WHERE name = 'cased'", []).unwrap();
    assert_eq!(n[0][0], rustqlite::Value::Integer(1));
    println!("[ok] case-insensitive identifiers");

    // 8. Transaction + rollback via fast path.
    db.execute("BEGIN", []).unwrap();
    db.execute("INSERT INTO t (name) VALUES ('tx1')", []).unwrap();
    db.execute("ROLLBACK", []).unwrap();
    let n = db.query("SELECT COUNT(*) FROM t WHERE name = 'tx1'", []).unwrap();
    ...
    let _ = std::fs::remove_file(&path);
    {
        ...
        db2.execute("CREATE TABLE d (id INTEGER PRIMARY KEY, name TEXT)", []).unwrap();
        db2.execute("INSERT INTO d (name) VALUES ('persisted')", []).unwrap();
    }
    {
