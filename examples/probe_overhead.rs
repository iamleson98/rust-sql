...
/// One (label, closure) benchmark case. The closure may borrow the
/// database under test, hence the lifetime parameter.
...
fn main() {
    ...
    let cases: Vec<Case<'_>> = vec![(
        "SELECT a FROM bench LIMIT 1",
        Box::new(|| {
            let _ = db.query("SELECT a FROM bench LIMIT 1", ());
        }),
    )];
    for (name, f) in cases {
        let t = Instant::now();
    ...
    for i in 0..n {
        let _ = db.query(
            ...
            [Value::Integer((i % 5000) as i64)],
        );
    }
    println!(
        "{:40} {:>7.2} µs/q",
        "SELECT a WHERE id=? (fast path)",
        t.elapsed().as_secs_f64() * 1e6 / n as f64
    );
