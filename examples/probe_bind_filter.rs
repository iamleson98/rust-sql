...
fn main() {
    ...
    println!(
        "literal BETWEEN + COUNT/AVG : {:>8.1} ms ({:>6.1} µs/q)",
    ...
    println!(
        "bind    BETWEEN + COUNT/AVG : {:>8.1} ms ({:>6.1} µs/q)",
        t.elapsed().as_secs_f64() * 1e3,
        t.elapsed().as_secs_f64() * 1e6 / n as f64
    );
    ...
    println!(
        "literal BETWEEN (no agg)    : {:>8.1} ms ({:>6.1} µs/q)",
        t.elapsed().as_secs_f64() * 1e3,
        t.elapsed().as_secs_f64() * 1e6 / n as f64
    );
    ...
    println!(
        "bind    BETWEEN (no agg)    : {:>8.1} ms ({:>6.1} µs/q)",
        t.elapsed().as_secs_f64() * 1e3,
        t.elapsed().as_secs_f64() * 1e6 / n as f64
    );
    ...
    println!(
        "bind    equality (no idx)   : {:>8.1} ms ({:>6.1} µs/q)",
        t.elapsed().as_secs_f64() * 1e3,
        t.elapsed().as_secs_f64() * 1e6 / n as f64
    );
    ...
    println!(
        "literal a > 4500 + COUNT    : {:>8.1} ms ({:>6.1} µs/q)",
        t.elapsed().as_secs_f64() * 1e3,
        t.elapsed().as_secs_f64() * 1e6 / n as f64
    );
    ...
    println!(
        "bind    a > 4500 + COUNT    : {:>8.1} ms ({:>6.1} µs/q)",
        t.elapsed().as_secs_f64() * 1e3,
        t.elapsed().as_secs_f64() * 1e6 / n as f64
    );
}
