...
fn bench_insert_autocommit() -> BenchResult {
    ...
    let (rs_ops, _) = measure("rusqlite autocommit insert", || {
        ...
        for i in 1..=n {
            conn.execute("INSERT INTO t (name, val) VALUES (?1, ?2)", params![format!("name{}", i), i]).unwrap();
        }
    }, n as usize);
    BenchResult { name: "INSERT (auto-commit, 1k rows)".into(), rustqlite_ops_per_sec: rq_ops, rusqlite_ops_per_sec: rs_ops, note: Some("fsync per stmt") }
}

fn bench_insert_transaction() -> BenchResult {
    let n = 1000;
    let (rq_ops, _) = measure("rust-sql txn insert", || {
        let mut db = Database::open_in_memory().unwrap();
        db.execute("CREATE TABLE t (id INTEGER PRIMARY KEY, name TEXT, val INTEGER)", []).unwrap();
        db.execute("BEGIN", []).unwrap();
        for i in 1..=n {
    ...
    let (rs_ops, _) = measure("rusqlite txn insert", || {
        let conn = rusqlite::Connection::open_in_memory().unwrap();
        conn.execute("CREATE TABLE t (id INTEGER PRIMARY KEY, name TEXT, val INTEGER)", []).unwrap();
        conn.execute("BEGIN", []).unwrap();
        for i in 1..=n {
    ...
    BenchResult { name: "INSERT (transaction, 1k rows)".into(), rustqlite_ops_per_sec: rq_ops, rusqlite_ops_per_sec: rs_ops, note: None }
}

fn bench_insert_multi_values() -> BenchResult {
    // Single INSERT with 1000 VALUES tuples — best case for bulk insert.
    let n = 1000;
    let (rq_ops, _) = measure("rust-sql multi-VALUES", || {
        let mut db = Database::open_in_memory().unwrap();
        db.execute("CREATE TABLE t (id INTEGER PRIMARY KEY, name TEXT, val INTEGER)", []).unwrap();
        ...
        while i <= n {
            ...
            let values: String = (i..=end).map(|j| format!("('name{}', {})", j, j))
                .collect::<Vec<_>>().join(",");
    ...
    let (rs_ops, _) = measure("rusqlite multi-VALUES", || {
        let conn = rusqlite::Connection::open_in_memory().unwrap();
        conn.execute("CREATE TABLE t (id INTEGER PRIMARY KEY, name TEXT, val INTEGER)", []).unwrap();
        let chunk_size = 100;
        let mut i = 1;
    ...
    BenchResult { name: "INSERT (multi-VALUES 100/batch)".into(), rustqlite_ops_per_sec: rq_ops, rusqlite_ops_per_sec: rs_ops, note: Some("bulk path") }
}
...
fn bench_point_lookup() -> BenchResult {
    ...
    let (rq_ops, _) = measure("rust-sql point lookup", || {
        for i in 1..=n {
            let _ = db.query("SELECT name, val FROM t WHERE id = ?", [Value::Integer(i)]).unwrap();
        }
    }, n as usize);
...
fn bench_index_lookup() -> BenchResult {
    ...
    let n = 1000;
    let (rq_ops, _) = measure("rust-sql index lookup", || {
        for i in 1..=n {
            let _ = db.query("SELECT name FROM t WHERE val = ?", [Value::Integer(i)]).unwrap();
        }
    }, n as usize);
    let (rs_ops, _) = measure("rusqlite index lookup", || {
        for i in 1..=n {
...
fn bench_full_scan() -> BenchResult {
    ...
    let conn = setup_rusqlite(N_ROWS, false);
    let n = 50;
    let (rq_ops, _) = measure("rust-sql full scan", || {
        for _ in 0..n {
            let _ = db.query("SELECT * FROM t", []).unwrap();
        }
    }, N_ROWS as usize * n);
    let (rs_ops, _) = measure("rusqlite full scan", || {
        for _ in 0..n {
...
fn bench_aggregate_multi() -> BenchResult {
    ...
    let conn = setup_rusqlite(N_ROWS, false);
    let n = 500;
    let (rq_ops, _) = measure("rust-sql aggregate multi", || {
        for _ in 0..n {
            let _ = db.query("SELECT COUNT(*), SUM(val), MIN(val), MAX(val), AVG(val) FROM t", []).unwrap();
        }
    }, n as usize);
    let (rs_ops, _) = measure("rusqlite aggregate multi", || {
        for _ in 0..n {
...
fn bench_aggregate_with_where() -> BenchResult {
    ...
    let conn = setup_rusqlite(N_ROWS, false);
    let n = 500;
    let (rq_ops, _) = measure("rust-sql agg + WHERE", || {
        for _ in 0..n {
            let _ = db.query("SELECT SUM(val), COUNT(*), AVG(score) FROM t WHERE val > 5000", []).unwrap();
        }
    }, n as usize);
    let (rs_ops, _) = measure("rusqlite agg + WHERE", || {
        for _ in 0..n {
...
fn bench_group_by() -> BenchResult {
    ...
    let conn = setup_rusqlite(N_ROWS, false);
    let n = 50;
    let (rq_ops, _) = measure("rust-sql GROUP BY", || {
        for _ in 0..n {
            let _ = db.query("SELECT val / 100 AS bucket, COUNT(*) FROM t GROUP BY bucket", []).unwrap();
        }
    }, N_ROWS as usize * n);
    let (rs_ops, _) = measure("rusqlite GROUP BY", || {
        for _ in 0..n {
...
fn bench_update_by_id() -> BenchResult {
    ...
    let n = 1000;
    let (rq_ops, _) = measure("rust-sql update by id", || {
        for i in 1..=n {
            let mut db = db.write();
            db.execute("UPDATE t SET val = ? WHERE id = ?", [Value::Integer(i * 2), Value::Integer(i)]).unwrap();
        }
    }, n as usize);
    let (rs_ops, _) = measure("rusqlite update by id", || {
        for i in 1..=n {
            let conn = conn.lock().unwrap();
            conn.execute("UPDATE t SET val = ?1 WHERE id = ?2", params![i * 2, i]).unwrap();
        }
    }, n as usize);
    BenchResult { name: "UPDATE by id (1k updates)".into(), rustqlite_ops_per_sec: rq_ops, rusqlite_ops_per_sec: rs_ops, note: None }
}

fn bench_delete_insert_cycle() -> BenchResult {
    let n = 500;
    let (rq_ops, _) = measure("rust-sql delete+insert cycle", || {
    ...
    let (rs_ops, _) = measure("rusqlite delete+insert cycle", || {
        let conn = setup_rusqlite(N_ROWS, false);
        for i in 1..=n {
            conn.execute("DELETE FROM t WHERE id = ?1", params![i]).unwrap();
            conn.execute("INSERT INTO t (id, name, val, score) VALUES (?1, ?2, ?3, ?4)",
                params![i, format!("name{}", i), i, i as f64]).unwrap();
        }
    }, n as usize * 2);
    BenchResult { name: "DELETE + INSERT cycle (500 iters)".into(), rustqlite_ops_per_sec: rq_ops, rusqlite_ops_per_sec: rs_ops, note: None }
}
...
fn bench_concurrent_8_readers() -> BenchResult {
        ...
        for _ in 0..8 {
            ...
            handles.push(thread::spawn(move || {
                for i in 0..n_per_thread {
                    ...
                    let guard = db.read();
                    let _ = guard.query("SELECT name, val FROM t WHERE id = ?", [Value::Integer(id)]);
                }
            }));
        }
    ...
    BenchResult { name: "Concurrent reads (8 threads × 500 queries)".into(), rustqlite_ops_per_sec: rq_ops, rusqlite_ops_per_sec: rs_ops, note: Some("Rust: RwLock | SQLite: Mutex") }
}
...
fn bench_concurrent_16_readers() -> BenchResult {
    ...
    let (rq_ops, _) = measure("rust-sql 16 readers", || {
        let mut handles = Vec::new();
        for _ in 0..16 {
            let db = Arc::clone(&db);
            handles.push(thread::spawn(move || {
    ...
    let (rs_ops, _) = measure("rusqlite 16 readers mutex", || {
        let mut handles = Vec::new();
        for _ in 0..16 {
            let conn = Arc::clone(&conn);
            handles.push(thread::spawn(move || {
    ...
    BenchResult { name: "Concurrent reads (16 threads × 250 queries)".into(), rustqlite_ops_per_sec: rq_ops, rusqlite_ops_per_sec: rs_ops, note: None }
}
...
fn bench_mixed_rw() -> BenchResult {
    ...
    let (rq_ops, _) = measure("rust-sql mixed R/W", || {
        let mut handles = Vec::new();
        // 1 writer
        {
            let db = Arc::clone(&db);
            handles.push(thread::spawn(move || {
                for i in 0..n_per_thread {
                    ...
                    guard.execute("UPDATE t SET val = val + 1 WHERE id = ?", [Value::Integer(id)]).unwrap();
                }
            }));
        }
        // 4 readers
    ...
    let (rs_ops, _) = measure("rusqlite mixed R/W", || {
        let mut handles = Vec::new();
        // 1 writer
        {
            let conn = Arc::clone(&conn);
            handles.push(thread::spawn(move || {
                for i in 0..n_per_thread {
                    let id = (i % N_ROWS as usize) as i64 + 1;
                    let guard = conn.lock().unwrap();
                    guard.execute("UPDATE t SET val = val + 1 WHERE id = ?1", params![id]).unwrap();
                }
            }));
        }
        // 4 readers
    ...
    BenchResult { name: "Mixed R/W (4 readers + 1 writer)".into(), rustqlite_ops_per_sec: rq_ops, rusqlite_ops_per_sec: rs_ops, note: None }
}

fn bench_concurrent_writes() -> BenchResult {
    // 4 writer threads each doing 250 INSERTs in a transaction.
    let total_ops = 4 * 250;
    let (rq_ops, _) = measure("rust-sql 4 writers (serialized)", || {
    ...
    let (rs_ops, _) = measure("rusqlite 4 writers (serialized)", || {
    ...
    BenchResult { name: "Concurrent writes (4 writers × 250 INSERTs)".into(), rustqlite_ops_per_sec: rq_ops, rusqlite_ops_per_sec: rs_ops, note: Some("writers serialize") }
}
...
fn bench_hash_join() -> BenchResult {
    ...
    let n = 50;
    let (rq_ops, _) = measure("rust-sql self-join", || {
        for _ in 0..n {
            let _ = db.query("SELECT a.id, b.val FROM t a JOIN t b ON a.val = b.val", []).unwrap();
        }
    }, (N_ROWS / 10) as usize * n * 2);
    let (rs_ops, _) = measure("rusqlite self-join", || {
        for _ in 0..n {
...
fn main() {
    ...
    println!("============================================================");
    println!("Summary:");
    println!("  rust-sql wins: {}  | SQLite wins: {}  | ties: {} (of {} tests)", wins_rust, wins_sqlite, ties, results.len());
    println!();

    // Print headline wins (rust-sql > SQLite by significant margin)
    let mut headlines: Vec<&BenchResult> = results.iter()
    ...
    let mut gaps: Vec<&BenchResult> = results.iter()
    ...
    if !gaps.is_empty() {
        ...
        for g in gaps.iter().take(5) {
            println!("    {:<42}  ratio: {:.2}x (SQLite wins by {:.1}x)", g.name, g.ratio(), 1.0 / g.ratio());
        }
    }
}
