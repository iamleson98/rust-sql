...
fn main() {
    ...
    // 1. Basic view
    db.execute("CREATE VIEW big AS SELECT * FROM t WHERE val > 50", [])
        .unwrap();
    check(&db, "SELECT COUNT(*) FROM big", "[[Integer(5)]]");
    // 2. View with projection
    check(
        &db,
        "SELECT name FROM big WHERE val = 70",
        "[[Text(\"n7\")]]",
    );
    // 3. View joined with a table
    check(
        &db,
        "SELECT COUNT(*) FROM big JOIN t ON big.id = t.id",
        "[[Integer(5)]]",
    );
    // 4. View over view
    db.execute("CREATE VIEW bigger AS SELECT * FROM big WHERE val > 80", [])
        .unwrap();
    check(&db, "SELECT COUNT(*) FROM bigger", "[[Integer(2)]]");
    // 5. View with column list
    db.execute(
        "CREATE VIEW renamed(a, b) AS SELECT name, val FROM t WHERE id <= 3",
        [],
    )
    .unwrap();
    check(
        &db,
        "SELECT a, b FROM renamed WHERE b = 20",
        "[[Text(\"n2\"), Integer(20)]]",
    );
    // 6. Aggregate view
    db.execute(
        "CREATE VIEW stats AS SELECT COUNT(*) AS c, SUM(val) AS s FROM t",
        [],
    )
    .unwrap();
    check(
        &db,
        "SELECT c, s FROM stats",
        "[[Integer(10), Integer(550)]]",
    );
    // 7. View + GROUP BY
    db.execute(
        "CREATE VIEW byten AS SELECT val / 10 AS bucket, COUNT(*) AS c FROM t GROUP BY bucket",
        [],
    )
    .unwrap();
    ...
    check(
        &db,
        "SELECT COUNT(*) FROM big b WHERE b.val > 90",
        "[[Integer(1)]]",
    );
    // 11. CREATE VIEW IF NOT EXISTS
    db.execute("CREATE VIEW IF NOT EXISTS big AS SELECT 1", [])
        .unwrap();
    check(&db, "SELECT COUNT(*) FROM big", "[[Integer(5)]]");
    ...
    db2.execute("CREATE TABLE t (id INTEGER PRIMARY KEY, x INTEGER)", [])
        .unwrap();
    db2.execute("INSERT INTO t (x) VALUES (1), (2)", [])
        .unwrap();
    db2.execute("CREATE VIEW vx AS SELECT x FROM t", [])
        .unwrap();
    drop(db2);
