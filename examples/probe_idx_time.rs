...
fn main() {
    ...
    println!(
        "raw index lookup:  {:?}/op (hits={})",
        d1.as_nanos() / (n as u128),
        hits
    );
    ...
    for i in 0..n {
        ...
        for rid in rids {
            ...
            if let rustqlite::storage::btree::LookupResult::Found(_p) =
    ...
    println!(
        "index+table fetch: {:?}/op (rows={})",
        d2.as_nanos() / (n as u128),
        rows
    );
    ...
    for i in 0..n {
        let mut tbt = Btree::new(&pager, table_root, false);
        if let rustqlite::storage::btree::LookupResult::Found(_p) =
            tbt.lookup_table(1 + (i % 10000)).unwrap()
        {
            found_t += 1;
        }
    }
    ...
    println!(
        "raw table lookup: {:?}/op (found={})",
        dt.as_nanos() / (n as u128),
        found_t
    );
    ...
    for i in 0..n {
        let _ = db2
            .query(sql, [Value::Integer(((i % 1000) + 1) * 2)])
            .unwrap();
    }
