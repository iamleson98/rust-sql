        ...
        if let Ok(row) = rustqlite::storage::row_codec::decode_row(payload, 6, 0, None) {
            ...
            for v in &row {
                if let Value::Text(n) = v {
                    if n.as_str() == "idx_cat" {
                        if let Some(Value::Integer(rp)) =
                            row.iter().find(|v| matches!(v, Value::Integer(_)))
                        {
                            idx_root = *rp as u32;
                        }
                    }
    ...
    fn walk(
        ...
        match pt {
            PageType::LeafIndex => {
                ...
                let f =
                    rustqlite::storage::btree::Cell::decode(&borrowed.data[first..], pt, page_size)
                        .unwrap();
                let l =
                    rustqlite::storage::btree::Cell::decode(&borrowed.data[last..], pt, page_size)
                        .unwrap();
                ...
                eprintln!(
                    ...
                    page_id,
                    n,
                    &f.index_key()[..f.index_key().len().min(12)],
                    &l.index_key()[..l.index_key().len().min(12)]
                );
            }
            PageType::InteriorIndex => {
                ...
                let mut child_ids: Vec<u32> = Vec::new();
                {
                    for i in 0..n {
                        ...
                        let c = rustqlite::storage::btree::Cell::decode(
                            &borrowed.data[cell_ptr..],
                            pt,
                            page_size,
                        )
                        .unwrap();
                        eprintln!(
                            "{}  cell{}: left_child={} key_bytes={:?} rowid={}",
                            "  ".repeat(depth),
                            i,
                            c.left_child(),
                            &c.index_key()[..c.index_key().len().min(10)],
                            c.key()
                        );
