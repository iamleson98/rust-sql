...
fn main() {
    ...
    {
        let mut stmt = db
            .prepare("SELECT a FROM bench WHERE a BETWEEN ? AND ? LIMIT 1")
            .unwrap();
        let t = Instant::now();
        for _ in 0..n {
            let _ = stmt.bind_all(&[Value::Integer(0), Value::Integer(100)]);
            let _ = stmt.step();
        }
        println!(
            "prepare+step     : {:>7.2} µs/q",
            t.elapsed().as_secs_f64() * 1e6 / n as f64
        );
    }
    ...
    let t = Instant::now();
    for _ in 0..n {
        let _ = db.query("SELECT a FROM bench WHERE id BETWEEN 0 AND 1 LIMIT 1", ());
    }
    println!(
        "rowid BETWEEN    : {:>7.2} µs/q",
        t.elapsed().as_secs_f64() * 1e6 / n as f64
    );
    ...
    let t = Instant::now();
    for _ in 0..n {
        let _ = db.query("SELECT a FROM bench WHERE a = 0 LIMIT 1", ());
    }
    println!(
        "a=0 LIMIT 1      : {:>7.2} µs/q",
        t.elapsed().as_secs_f64() * 1e6 / n as f64
    );
}
