...
fn main() {
    // (a) point lookup as first query
    {
        ...
        println!("first=point-lookup   {:>8.1} µs", t0.elapsed().as_secs_f64() * 1e6);
    }
    // (b) COUNT(*) as first query (aggregate streaming path)
    {
        ...
        println!("first=COUNT(*)       {:>8.1} µs", t0.elapsed().as_secs_f64() * 1e6);
    }
    // (c) full scan first, then point lookup
    {
        ...
        let t1 = Instant::now();
        let _ = db.query("SELECT name, val, score FROM t WHERE id = ?", [Value::Integer(1)]).unwrap();
        println!("full-scan {:>8.1} µs, then point {:>8.1} µs",
    ...
    // (d) point lookup on an EMPTY table (parse+plan+fastpath only)
    {
        ...
        db.set_deferred_flush(true);
        db.execute("CREATE TABLE t (id INTEGER PRIMARY KEY, name TEXT, val INTEGER, score REAL)", []).unwrap();
        db.execute("INSERT INTO t VALUES (1, 'x', 1, 1.0)", []).unwrap();
        let t0 = Instant::now();
        let _ = db.query("SELECT name, val, score FROM t WHERE id = ?", [Value::Integer(1)]).unwrap();
        println!("first=point(1 row)   {:>8.1} µs", t0.elapsed().as_secs_f64() * 1e6);
    }
}
