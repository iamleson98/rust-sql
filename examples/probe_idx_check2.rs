fn main() {
    ...
    let tests = [
        ...
        ("val >= 90", "SELECT COUNT(*) FROM t WHERE val >= 90"),
        (
            "val BETWEEN 10 AND 20",
            "SELECT COUNT(*) FROM t WHERE val BETWEEN 10 AND 20",
        ),
        (
            "val < 10 (no index?)",
        ...
        (
            "non-sargable val+0 > 90",
            "SELECT COUNT(*) FROM t WHERE val + 0 > 90",
        ),
        (
    ...
    for (name, sql) in tests {
        ...
        println!(
            "{:<28} -> {:?}",
            name,
            r.iter().map(|row| row[0].clone()).collect::<Vec<_>>()
        );
    }
    // Full scan check.
