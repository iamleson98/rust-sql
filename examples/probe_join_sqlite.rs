...
fn main() {
    ...
    for _ in 0..100 { let _ = db.query(SQL, [Value::Integer(1)]).unwrap(); }
    let n = 500;
    let start = Instant::now();
    for _ in 0..n { let _ = db.query(SQL, [Value::Integer(1)]).unwrap(); }
    println!("rustqlite steady (id=1):  {:>7.1} us/op", us(start.elapsed()) / n as f64);

    // single-shot with the SAME param (data warm) — isolates fixed cost
    let start = Instant::now();
    let rows = db.query(SQL, [Value::Integer(1)]).unwrap();
    println!("rustqlite single (warm):   {:>7.1} us ({} rows)", us(start.elapsed()), rows.len());
    ...
    let mut stmt_i = conn.prepare("INSERT INTO users (name, dept) VALUES (?1, 'eng')").unwrap();
    for i in 1..=1000i64 {
        stmt_i.execute(rusqlite::params![format!("user{}", i)]).unwrap();
    }
    let mut stmt_o = conn.prepare("INSERT INTO orders (user_id, total) VALUES (?1, ?2)").unwrap();
    for i in 1..=10000i64 {
        stmt_o.execute(rusqlite::params![(i % 1000) + 1, i * 10]).unwrap();
    }
    let mut stmt_it = conn.prepare("INSERT INTO items (order_id, name, price) VALUES (?1, ?2, ?3)").unwrap();
    for i in 1..=50000i64 {
        stmt_it.execute(rusqlite::params![(i % 10000) + 1, format!("item{}", i), i as f64 * 0.5]).unwrap();
    }
    ...
    println!("sqlite steady (id=1):     {:>7.1} us/op", us(start.elapsed()) / n as f64);
    ...
    let cnt2 = {
        ...
        let mut c = 0; while rows.next().unwrap().is_some() { c += 1 } c
    };
    println!("sqlite single (id=500 cold):    {:>7.1} us ({} rows)", us(start.elapsed()), cnt2);

    // and again now that 500 is warm
    let start = Instant::now();
    let _ = db.query(SQL, [Value::Integer(500)]).unwrap();
    println!("rustqlite single (id=500 warm): {:>7.1} us", us(start.elapsed()));
    let start = Instant::now();
    let mut rows = stmt.query(rusqlite::params![500]).unwrap();
    while rows.next().unwrap().is_some() {}
    println!("sqlite single (id=500 warm):    {:>7.1} us", us(start.elapsed()));
}
