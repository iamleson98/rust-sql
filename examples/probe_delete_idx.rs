...
fn main() {
    ...
    for i in 9000..9100i64 {
        db.execute("DELETE FROM t WHERE id = ?", [rustqlite::Value::Integer(i)])
            .unwrap();
    }
    ...
    println!(
        "DELETE by PK w/ index (1k ops): {:?} ({:.2?}/op)",
        start.elapsed(),
        start.elapsed() / 1000
    );
    ...
    let v = db
        .query("SELECT COUNT(*) FROM t WHERE val > 10000", [])
        .unwrap();
    println!(
        "indexed count (val > 10000, expect 10000-9900... ): {:?}",
        v[0][0]
    );
    let row = db
        .query("SELECT id, name, val FROM t WHERE id = 5000", [])
        .unwrap();
