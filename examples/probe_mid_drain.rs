...
fn main() {
    ...
    println!("mid-storm drain, query {:>7.1} µs", t0.elapsed().as_secs_f64() * 1e6);
    ...
    drain();
    let t1 = Instant::now();
    let _ = db2.query("SELECT name, val, score FROM t WHERE id = ?", [Value::Integer(1)]).unwrap();
    println!("post-storm drain, query {:>7.1} µs", t1.elapsed().as_secs_f64() * 1e6);
}
