...
fn main() {
    ...
    .unwrap();
    db.execute("INSERT INTO t (name, val, score) VALUES ('a', 1, 1.0)", [])
        .unwrap();
    ...
    for _ in 0..100 {
        let _ = db
            .query(
                sql_between,
                [rustqlite::Value::Integer(1), rustqlite::Value::Integer(1)],
            )
            .unwrap();
    }
    for _ in 0..100 {
        let _ = db.query(sql_point, [rustqlite::Value::Integer(1)]).unwrap();
    }
