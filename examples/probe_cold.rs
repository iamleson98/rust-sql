...
fn main() {
    ...
    println!(
        "idx-point variant 1st: {:>8.1} us  (parse+plan only, cache warm path)",
        us(start.elapsed())
    );
    ...
    println!(
        "idx-point variant avg: {:>8.1} us  (20 fresh parses+plans)",
        total / 20.0
    );
    ...
    let start = Instant::now();
    let _ = db
        .query(sql4, [Value::Integer(1000), Value::Integer(1009)])
        .unwrap();
    ...
    let start = Instant::now();
    for _ in 0..n {
        let _ = db.query(sql5, [Value::Integer(500)]).unwrap();
    }
    println!(
        "join3 steady (id=500): {:>8.1} us/op",
