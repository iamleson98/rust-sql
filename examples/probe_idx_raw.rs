...
fn main() {
    ...
    for pid in 0..pager.n_pages() {
        ...
        eprintln!("page {}: {:?} n_cells={} right={}",
            pid, pt, b.n_cells(), b.right_most_pointer());
        ...
        if matches!(pt, PageType::InteriorIndex | PageType::InteriorTable) {
            ...
            for i in 0..n {
                ...
                eprintln!("  cell{}: left_child={} sep_key(hex)={} sep_key(txt)={:?} sep_rowid={}",
                    i, c.left_child(), keyhex, keytxt, c.key());
            }
        }
