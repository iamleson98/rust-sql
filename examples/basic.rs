...
fn main() {
    ...
    // Query all rows.
    println!("All users:");
    let rows = db.query("SELECT id, name, email, age FROM users ORDER BY age", []).unwrap();
    for row in &rows {
    ...
    // Query with a filter.
    println!("\nUsers over 28:");
    let rows = db.query("SELECT name, age FROM users WHERE age > 28 ORDER BY age", []).unwrap();
    for row in &rows {
    ...
    // Aggregate.
    println!("\nStats:");
    let rows = db.query("SELECT COUNT(*), MIN(age), MAX(age), AVG(age) FROM users", []).unwrap();
    let row = &rows[0];
    println!("  count={} min={} max={} avg={}", row[0], row[1], row[2], row[3]);

    // Update.
    db.execute("UPDATE users SET age = 31 WHERE name = 'Alice'", []).unwrap();
    let rows = db.query("SELECT name, age FROM users WHERE name = 'Alice'", []).unwrap();
    println!("\nAfter update: Alice is now {}", rows[0][1]);

    // Delete.
    db.execute("DELETE FROM users WHERE name = 'Bob'", []).unwrap();
    ...
    let rows = db
        .query("SELECT name FROM users WHERE age > ?", vec![Value::Integer(30)])
        .unwrap();
    for row in &rows {
        println!("  {}", row[0]);
    }
