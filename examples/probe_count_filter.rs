...
fn main() {
    ...
    for _ in 0..10 {
        let r = db.query("SELECT COUNT(*) FROM t WHERE val > 5000", []).unwrap();
        assert_eq!(r[0][0].as_integer(), 5000);
    }
    println!("COUNT(*) WHERE val>5000 x10: {:?} ({:?}/query)", start.elapsed(), start.elapsed() / 10);
    ...
    println!("COUNT(*) x10: {:?} ({:?}/query)", start.elapsed(), start.elapsed() / 10);
    ...
    println!("SUM/AVG/MIN/MAX x10: {:?} ({:?}/query)", start.elapsed(), start.elapsed() / 10);
    ...
    println!("sqlite COUNT(*) WHERE x10: {:?} ({:?}/query)", start.elapsed(), start.elapsed() / 10);
}
