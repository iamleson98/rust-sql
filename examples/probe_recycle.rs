...
fn main() {
    ...
    println!(
        "after 1st 10k insert: {} KB, pages = {}",
        size_after_first / 1024,
        db.page_count()
    );
    ...
    println!(
        "after 2nd 10k insert: {} KB, pages = {}",
        size_after_second / 1024,
        db.page_count()
    );
    ...
    if size_after_second <= size_after_first + 16384 {
        println!(
            "PASS: file did not grow ({} -> {} bytes)",
            size_after_first, size_after_second
        );
    } else {
        println!(
            "FAIL: file grew {} -> {} bytes",
            size_after_first, size_after_second
        );
    }
}
