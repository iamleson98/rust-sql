...
fn main() {
    ...
    println!("first query:   {:>8.1} µs", t0.elapsed().as_secs_f64() * 1e6);
    // second query — cache populate
    let t1 = Instant::now();
    let _ = db.query(qsql, [Value::Integer(2)]).unwrap();
    println!("second query:  {:>8.1} µs", t1.elapsed().as_secs_f64() * 1e6);
    // third query — warm
    let t2 = Instant::now();
    let _ = db.query(qsql, [Value::Integer(3)]).unwrap();
    println!("third query:   {:>8.1} µs", t2.elapsed().as_secs_f64() * 1e6);
    // 1000 warm ops
    let t3 = Instant::now();
    for i in 1..=1000i64 {
        let target = (i % 1000) + 1;
    ...
    println!("1000 warm ops: {:>8.1} µs ({:.1} ns/op)",
