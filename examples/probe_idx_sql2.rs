fn main() {
    ...
    for (lo, hi) in [
        ...
        let r = db
            .query(
                "SELECT COUNT(*) FROM t WHERE val > ? AND val <= ?",
                [rustqlite::Value::Integer(lo), rustqlite::Value::Integer(hi)],
            )
            .unwrap();
    ...
    // Ordered index scan.
    let r = db
        .query("SELECT val FROM t WHERE val > 0 ORDER BY val LIMIT 3", [])
        .unwrap();
    println!("first 3 by val: {:?}", r);
    let r = db
        .query(
            "SELECT val FROM t WHERE val > 1200 ORDER BY val LIMIT 3",
            [],
        )
        .unwrap();
