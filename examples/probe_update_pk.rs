...
fn main() {
    ...
    println!("rustqlite UPDATE by PK 1k: {:?} ({} ns/op)", d_r, d_r.as_nanos() as f64 / 1000.0);
    ...
    for i in 1..=50 {
        conn.execute("UPDATE t SET score = ? WHERE id = ?", rusqlite::params![i as f64, i]).unwrap();
    }
    let start = Instant::now();
    for i in 51..=1050 {
        conn.execute("UPDATE t SET score = ? WHERE id = ?", rusqlite::params![i as f64, i]).unwrap();
    ...
    println!("sqlite     UPDATE by PK 1k: {:?} ({} ns/op)", d_s, d_s.as_nanos() as f64 / 1000.0);
