...
fn main() {
    ...
    // 1. Basic CTE
    check(&db, "WITH big AS (SELECT * FROM t WHERE val > 50) SELECT COUNT(*) FROM big", "[[Integer(5)]]");
    // 2. CTE with projection + outer filter
    check(&db, "WITH v AS (SELECT id, val FROM t) SELECT SUM(val) FROM v WHERE id <= 4", "[[Integer(100)]]");
    ...
    check(&db, "WITH c(x) AS (SELECT val FROM t WHERE id = 1) SELECT x FROM c", "[[Integer(10)]]");
    // 5. CTE referenced twice
    check(&db, "WITH d AS (SELECT val FROM t) SELECT (SELECT MAX(val) FROM d) - (SELECT MIN(val) FROM d)", "[[Integer(90)]]");
    ...
    check(&db, "WITH t AS (VALUES(42)) SELECT * FROM t", "[[Integer(42)]]");
    ...
    // 13. Params in CTE
    let r = db.query("WITH p AS (SELECT val FROM t WHERE val > ?) SELECT COUNT(*) FROM p", [rustqlite::Value::Integer(50)]).unwrap();
    println!("[{}] params in CTE: {:?} (expect 5)", if r == vec![vec![rustqlite::Value::Integer(5)]] { "ok" } else { "FAIL" }, r);
    ...
    for _ in 0..3 {
        let r = db.query("WITH rr AS (SELECT COUNT(*) AS c FROM t) SELECT c FROM rr", []).unwrap();
    ...
    check(&db, "WITH m AS MATERIALIZED (SELECT val FROM t WHERE id = 2) SELECT val FROM m", "[[Integer(20)]]");
