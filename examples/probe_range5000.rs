...
fn main() {
    ...
    // warm
    let _ = db
        .query(sql, [Value::Integer(1000), Value::Integer(5999)])
        .unwrap();
    let n = 300;
    ...
    println!(
        "full 5000-row query:      {:>8.1} us  ({:.1} ns/row)",
        full,
        full * 1000.0 / 5000.0
    );
    ...
    let _ = db
        .query(sql2, [Value::Integer(1000), Value::Integer(5999)])
        .unwrap();
    ...
    println!(
        "COUNT same range:         {:>8.1} us  ({:.1} ns/row)",
        count,
        count * 1000.0 / 5000.0
    );
    ...
    let _ = db
        .query(sql3, [Value::Integer(1000), Value::Integer(5999)])
        .unwrap();
    ...
    println!(
        "SUM(val) same range:      {:>8.1} us  ({:.1} ns/row)",
        sum1,
        sum1 * 1000.0 / 5000.0
    );
    ...
    for (lo, hi, label) in [(1000i64, 1099i64, "100"), (1000, 1999, "1000")] {
        ...
        println!(
            "full {}-row query:      {:>8.1} us  ({:.1} ns/row)",
            label,
            t,
            t * 1000.0 / w
        );
    ...
    for _ in 0..n {
        keep = Some(
            db.query(sql, [Value::Integer(1000), Value::Integer(5999)])
                .unwrap(),
        );
    }
    println!(
        "(query+keep last:        {:>8.1} us)",
