fn main() {
    ...
    for page_size in [8192u32, 4096] {
        ...
        db.execute(&format!("PRAGMA page_size = {page_size}"), []).unwrap();
        ...
        for i in 0..10000i64 {
            db.execute("INSERT INTO t (name, val, score) VALUES (?, ?, ?)",
                [rustqlite::Value::Text(format!("item_{i}").into()), rustqlite::Value::Integer(i), rustqlite::Value::Real(i as f64)]).unwrap();
        }
        db.execute("COMMIT", []).unwrap();

        let t = std::time::Instant::now();
        for _ in 0..5 { db.execute("UPDATE t SET score = score + 1.0 WHERE val > 5000", []).unwrap(); }
        let upd = t.elapsed().as_secs_f64() * 1e3 / 5.0;

        let t = std::time::Instant::now();
        for _ in 0..50 { let _ = db.query("SELECT COUNT(*), SUM(val) FROM t WHERE val > 5000", ()); }
        let agg = t.elapsed().as_secs_f64() * 1e3 / 50.0;

        let t = std::time::Instant::now();
        for _ in 0..50 { let _ = db.query("SELECT name FROM t WHERE id = 5000", ()); }
