...
fn main() {
    ...
    println!(
        "After load (10k rows): page_count = {}, cache = {:?}",
        db.page_count(),
        db.cache_stats()
    );
    ...
    println!(
        "After 1k updates to 100 rowids: page_count = {}, took {:?} ({:?}/op)",
        db.page_count(),
        dur,
        dur / 1000
    );
    ...
    println!(
        "After 1k updates to 1000 distinct rowids: page_count = {}, took {:?} ({:?}/op)",
        db.page_count(),
        dur2,
        dur2 / 1000
    );
    ...
    println!(
        "After 5k updates to 5000 distinct rowids: page_count = {}, took {:?} ({:?}/op)",
        db.page_count(),
        dur3,
        dur3 / 5000
    );
