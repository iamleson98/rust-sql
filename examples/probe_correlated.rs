...
fn main() {
    ...
    db.execute("CREATE TABLE cakes (id INTEGER PRIMARY KEY, name TEXT)", []).unwrap();
    db.execute("CREATE TABLE bakers (id INTEGER PRIMARY KEY, name TEXT)", []).unwrap();
    db.execute("CREATE TABLE cake_baker (cake_id INT, baker_id INT)", []).unwrap();
    db.execute("INSERT INTO cakes VALUES (1,'C1'), (2,'C2'), (3,'C3')", []).unwrap();
    db.execute("INSERT INTO bakers VALUES (10,'B10'), (11,'B11')", []).unwrap();
    db.execute("INSERT INTO cake_baker VALUES (1,10), (2,11)", []).unwrap();
    ...
    println!("in subquery:        {}", q(&mut db, "SELECT * FROM cake_baker WHERE cake_id IN (SELECT id FROM cakes WHERE name LIKE 'C%')"));
}
