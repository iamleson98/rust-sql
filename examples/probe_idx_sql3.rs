fn main() {
    ...
    // In the SAME session:
    let r = db.query("SELECT COUNT(*) FROM t WHERE val = 1236", []).unwrap();
    ...
    let r2 = db2.query("SELECT COUNT(*) FROM t WHERE val = 1236", []).unwrap();
    println!("reopened   val=1236: {:?} (expect 1)", r2[0][0]);
    let r3 = db2.query("SELECT COUNT(*) FROM t WHERE val > 0", []).unwrap();
