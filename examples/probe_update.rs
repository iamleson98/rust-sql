...
fn main() {
    ...
    // warm
    for i in 0..50 {
        db.execute(sql, [Value::Real(i as f64), Value::Integer((i % 1000) + 1)])
            .unwrap();
    }
    ...
    for i in 0..n as i64 {
        db.execute(
            sql,
            [Value::Real(i as f64 * 2.5), Value::Integer((i % 1000) + 1)],
        )
        .unwrap();
    }
    let total = start.elapsed();
    println!(
        "UPDATE by PK: {:.0} ns/op",
    ...
    db2.execute("CREATE TABLE t_del (id INTEGER PRIMARY KEY, x INTEGER)", [])
        .unwrap();
    db2.execute("BEGIN", []).unwrap();
    for i in 1..=2000i64 {
        db2.execute("INSERT INTO t_del (x) VALUES (?)", [Value::Integer(i)])
            .unwrap();
    }
    ...
    let start = Instant::now();
    for i in 0..500i64 {
        db2.execute(sql2, [Value::Integer((i % 1900) + 30)])
            .unwrap();
    }
    println!(
        "DELETE by PK: {:.0} ns/op",
        start.elapsed().as_nanos() as f64 / 500.0
    );
}
