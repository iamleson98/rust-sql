...
fn main() {
    ...
    let idx_cnt: i64 = db.query("SELECT COUNT(*) FROM t WHERE val > 0", []).unwrap()[0][0].as_integer();
    println!("rustqlite multirow 10k (indexed): {:?} rows={} idx_scan={}", d_r, cnt, idx_cnt);
    ...
    println!("ratio: {:.2}x {}", if d_r < d_s {"FASTER"} else {"slower"}, d_s.as_secs_f64() / d_r.as_secs_f64());
    ...
    for chunk in vals.chunks(500) {
        let values: Vec<String> = chunk.iter().map(|v| format!("('n{}', {}, 1.5)", v, v)).collect();
        db2.execute(&format!("INSERT INTO t (name, val, score) VALUES {}", values.join(", ")), []).unwrap();
    }
    let d_rand = start.elapsed();
    let cnt2: i64 = db2.query("SELECT COUNT(*) FROM t WHERE val BETWEEN 100 AND 200", []).unwrap()[0][0].as_integer();
    println!("rustqlite random-order 10k: {:?} (sanity idx count={})", d_rand, cnt2);
    ...
    for chunk in vals.chunks(500) {
        let values: Vec<String> = chunk.iter().map(|v| format!("('n{}', {}, 1.5)", v, v)).collect();
        conn2.execute(&format!("INSERT INTO t (name, val, score) VALUES {}", values.join(", ")), []).unwrap();
    }
    let d_rand_s = start.elapsed();
    println!("sqlite     random-order 10k: {:?} (ratio {:.2}x)", d_rand_s, d_rand_s.as_secs_f64() / d_rand.as_secs_f64());
}
